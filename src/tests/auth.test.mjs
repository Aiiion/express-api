...
describe('Auth Routes', () => {
  ...
  describe('POST /v1/auth/login', () => {
    ...
    it('should return 401 when password is incorrect', async () => {
      const response = await request(app).post('/v1/auth/login').send({ password: 'wrong-password' });
      expect(response.status).toBe(401);
    });

    it('should return 200 and sessionToken when password is correct', async () => {
      const response = await request(app).post('/v1/auth/login').send({ password: 'test-password' });
  ...
  describe('POST /v1/auth/verify', () => {
    ...
    it('should return 401 when code is incorrect', async () => {
      ...
      const loginResponse = await request(app).post('/v1/auth/login').send({ password: 'test-password' });

      const { sessionToken } = loginResponse.body;

      const response = await request(app).post('/v1/auth/verify').send({ sessionToken, code: '000000' });
    ...
    it('should return 200 and JWT token when verification succeeds', async () => {
      ...
      await setJsonValue(
        `auth_session_${testSessionToken}`,
        {
          code: testCode,
          createdAt: Date.now(),
        },
  ...
  describe('GET /v1/auth/verify-token', () => {
    ...
    it('should return 401 when token is invalid', async () => {
      const response = await request(app).get('/v1/auth/verify-token').set('Cookie', 'jwt_token=invalid-token');
      expect(response.status).toBe(401);
    ...
    it('should return 200 when token is valid', async () => {
      ...
      const response = await request(app).get('/v1/auth/verify-token').set('Cookie', jwtCookie);
  ...
  describe('GET /v1/requestLogs/meta/:field', () => {
    it('should return 404 when field is not a valid log column', async () => {
      const response = await request(app).get('/v1/requestLogs/meta/not_a_column').set('Cookie', getAuthCookie());
  ...
  describe('GET /v1/requestLogs', () => {
    it('should filter by a single code', async () => {
      const findAndCountAllSpy = jest
        .spyOn(sequelize.models.RequestLog, 'findAndCountAll')
        .mockResolvedValue({ count: 1, rows: [{ id: 1, code: 404 }] });

      const response = await request(app).get('/v1/requestLogs').query({ code: '404' }).set('Cookie', getAuthCookie());
    ...
    it('should filter by multiple codes', async () => {
      const findAndCountAllSpy = jest.spyOn(sequelize.models.RequestLog, 'findAndCountAll').mockResolvedValue({
        count: 2,
        rows: [
          { id: 1, code: 400 },
          { id: 2, code: 404 },
  ...
  describe('GET /v1/errorLogs/meta/:field', () => {
    it('should return 404 when field is not a valid error log column', async () => {
      const response = await request(app).get('/v1/errorLogs/meta/not_a_column').set('Cookie', getAuthCookie());
  ...
  describe('GET /v1/errorLogs', () => {
    ...
    it('should return 200 with paginated results', async () => {
      const findAndCountAllSpy = jest
        .spyOn(sequelize.models.ErrorLog, 'findAndCountAll')
        .mockResolvedValue({ count: 1, rows: [{ id: 1, message: 'Test error' }] });

      const response = await request(app).get('/v1/errorLogs').set('Cookie', getAuthCookie());
    ...
    it('should return correct pagination metadata', async () => {
      ...
      const response = await request(app).get('/v1/errorLogs').query({ page: '3' }).set('Cookie', getAuthCookie());
