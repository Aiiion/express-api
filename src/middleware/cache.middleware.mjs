      ...
      res.send = body => {
        if (res.statusCode < 500) {
          ...
          if (typeof body === 'string') {
            try {
              valueToCache = JSON.parse(body);
            } catch {
              /* not JSON, skip caching */ valueToCache = null;
            }
          }
          if (valueToCache !== null) {
            setJsonValue(key, valueToCache, duration).catch(err => devError('Failed to write cache entry:', err));
          }
        }
