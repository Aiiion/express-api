...
- Precipitation is merged by **rate**, not by raw total. `adjustPrecipitationAcrossHours` buckets hours into windows the width of the coarsest provider's period, averages each source's mm/h over the window (a source absent from the window is skipped; a source forecasting 0 mm counts), then redistributes `rate × windowHours` across the window's entries. `hours_measured` on the output states how much of the window each entry stands for, so a window with fewer entries than hours keeps its total intact
- Weather condition text is merged by consensus over a shared vocabulary, not by string comparison. Each DTO maps its provider's *structured* code — OWM `weather[0].id`, WeatherAPI `condition.code`, SMHI Wsymb2, MET `symbol_code` — onto a `condition` code from `src/utils/weatherConditions.mjs`; the aggregator takes a majority vote on the condition's coarse `group`, then the median intensity `rank` within it, and sources `weather`/`description`/`icon` from a provider that agreed. Never map from a provider's display text — the four vocabularies share no strings
