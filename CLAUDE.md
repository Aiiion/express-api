...
- `sunrise`/`sunset` on `currentWeather` (epoch seconds) come only from WeatherAPI, and only from its *forecast* payload — `forecast.forecastday[].astro` holds them as local clock text (`"06:14 AM"`, or `"No sunrise"` above the polar circles); `current.json` has none, and SMHI/MET never report them. So `weatherApiDto.currentWeather(data, metric, forecast)` takes the raw forecast.json as a third argument, picks the day matching the location's current local date and converts the text with `localTimeToEpoch` in `dateTimeHelpers.mjs`. `allWeather()` reuses the forecast it already fetched; the standalone `currentWeather()` fetches a one-day forecast for the purpose. If the forecast call fails the fields are null, not an error
