...
- Every DTO stamps an entry's precipitation as the period *starting* at `dt` (`[dt, dt + hours_measured)`), which is what the aggregator assumes. WeatherAPI and MET report that way natively; OWM (`rain.3h` = "last 3 hours") and SMHI (`time` closes the interval begun at `intervalParametersStartTime`) stamp at the *end*, so their forecast DTOs hand each entry the following entry's precipitation and drop the final entry, which has no successor
...
- Weather condition text is merged by consensus over a shared vocabulary, not by string comparison. Each DTO maps its provider's *structured* code — OWM `weather[0].id`, WeatherAPI `condition.code`, SMHI Wsymb2, MET `symbol_code` — onto a `condition` code from `src/utils/weatherConditions.mjs`; the aggregator takes a majority vote on the condition's coarse `group` (an even dry-versus-wet split is settled by the voters' own precipitation amounts — ≥ 0.1 mm/h counts as wet — so the condition never contradicts the amount beside it; what the amounts can't separate goes to the more severe group per `GROUP_SEVERITY`, never to whichever provider is listed first), then the median intensity `rank` within it, sources `weather`/`description` from a provider that agreed, and builds `icon` from the consensus condition in WeatherAPI's URL format (`weatherApiIconFor`; day/night is read from any provider icon, so it is null when only SMHI/MET responded). Never map from a provider's display text — the four vocabularies share no strings
...
- Fixtures (`src/fixtures/`) and DTOs (`src/dtos/`) pair one-to-one per weather provider. A DTO feeding the aggregator must emit a `condition` code from `src/utils/weatherConditions.mjs`; without it the provider abstains from the condition vote and falls back to generic string merging.
